...
import { fmt, fromMm, MM_PER_OZ, si } from '../lib/units';
        ...
        <div className="min-w-0 space-y-4">
          {r && (
            <Panel title={`At ${fmt(p.f, 4)} MHz`}>
              <table className="tbl">
                <tbody>
                  <Result label="Skin depth δ" value={`${fmt(fromMm(r.skinDepthMm, unit === 'mil' ? 'mil' : 'um'), 4)}`} unit={unit === 'mil' ? 'mil' : 'µm'} strong />
...
function Method() {
  return (
    <>
      ...
      <p>
        For a trace over a plane, the current concentrates on the side facing the plane, and surface roughness adds more loss above a few GHz. The real resistance can be higher than
        this estimate. Use it for trends and sizing, not for loss budgets at multi-GHz.
      </p>
      <h2>References</h2>
      <ol>
